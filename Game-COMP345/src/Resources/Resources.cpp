...
void deck::Tile::printInfo() {
  if (this) {
    std::cout << "Card: " << resources->at(0) << resources->at(1)
              << resources->at(2) << resources->at(3) << std::endl;
  } else {
...
void deck::Hand::drawTile() { HarvestHand->push_back(_HarvestDeck->draw()); }
...
void deck::Hand::refreshField() { _BuildingDeck->refreshBoardField(); }

int deck::Hand::countHarvestHand() { return HarvestHand->size(); }
...
void deck::Hand::displayField() { _BuildingDeck->printField(); }
...
std::pair<int, deck::Tile*> deck::Hand::exchange() {
  ...
  Tile* temp =
      HarvestHand->at(selection);  // <- make sure this doesnt get deleted since
                                   // contains pointer also used in HarvestHand
  // Last 2 getting deleted at end of current scope, since not pointers
  // might need assignment to make deep copy

  // is functional if remove the destructor for Tile, but leaves us with memory
  // leaks
...
void deck::Hand::drawShipment() { shipmentTile = _HarvestDeck->drawShipment(); }
